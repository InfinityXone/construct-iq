#!/usr/bin/env bash
# finish_swarm_import_v2.sh — robust first import of infinity-x-one-swarm into construct-iq
# - First import: git subtree add (preserve history)
# - Fallback: subtree-merge strategy (merge -s ours + read-tree --prefix)
# - Move mapped folders; namespace CI; commit & push; smoke summary
...
# make sure we’re on a feature branch already (you created one earlier)
CUR_BRANCH="$(git rev-parse --abbrev-ref HEAD)"
[ "$CUR_BRANCH" = "main" ] && die "Switch to your merge/* branch first (you’re on main)."
...
# first import: subtree add (if no staging dir yet)
subtree_add() {
  say "first import: git subtree add --prefix=$STAGING_PREFIX $SWARM_REMOTE $SWARM_BRANCH"
  git subtree add --prefix="$STAGING_PREFIX" "$SWARM_REMOTE" "$SWARM_BRANCH" \
    -m "feat(subtree): add $SWARM_REMOTE@$SWARM_BRANCH"
}
subtree_merge_fallback() {
...
# Namespace CI workflows safely (avoid bad redirection in the for-list; use nullglob)
if [ -d "${STAGING_PREFIX}/ci/github-actions" ]; then
  ...
  # if no *.yml/*.yaml, don't leave literals in the loop
  shopt -s nullglob
  for f in "$tmpdir"/*.yml "$tmpdir"/*.yaml; do
    base="$(basename "$f")"
    dest=".github/workflows/swarm-${base}"
    mv "$f" "$dest"
    git add "$dest"
  done
  shopt -u nullglob
...
say "pushing current branch -> $PUSH_REMOTE"
git push -u "$PUSH_REMOTE" "$(git rev-parse --abbrev-ref HEAD)"

echo
echo "[✓] Review branch pushed — open PR into main"
